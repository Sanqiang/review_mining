...
public class SentEva {
	...
	public static void evaluateMovieReview() throws Exception {
		...
		if (new File(map_all_obj).exists()) {
			...
			Graph graph = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 10, false, 25, 4);
		...
		if (new File(map_reduce_obj).exists()) {
			...
			Graph graph1 = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 0.0166040503, false, 25, 4);
		...
		Graph graph2 = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 0.226, true, 25, 4);
	...
	public static void evaluateCoconutWater() throws Exception {
		...
		if (new File(map_all_obj).exists()) {
			...
			Graph graph = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 10, false, 20, 5);
...
//			oin.close();
//		} else {
			Graph graph1 = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 0.1075069783, false, 20, 5);
		...
		Graph graph2 = Report.readData(Config.PATH_TEXT, Config.PATH_WEIGHT, 0.1, true, 20, 5);
